...
def natural_to_standard(nat_param, p):
    ...
    m = b / kappa
    # S: [..., p, p]
    S = A - outer(b, m)  # + 1e-5*tf.eye(p)
    # make the negative diagnal entries as positive epsilon 1e-5
...
def log_partition(nat_param, p):
    ...
    #S = S + 1e-5 * tf.eye(p)
    #s, u, v = tf.svd(S)
    #idx_ann = tf.where(tf.greater_equal(0,s))
    #reciprocal = tf.where(idx_ann, tf.reciprocal(s), tf.zeros(s.shape))


    # ret: [...]
    a1 = 0.5 * tf.squeeze(nu, -1)
    a2 = (p * tf.log(2.) - tf.linalg.logdet(S))
    a3 = log_multi_gamma(0.5 * nu, p) 
    a4 = 0.5 * p * tf.log(kappa)
    return  a1*a2 + a3 - a4
    #return 0.5 * tf.squeeze(nu, -1) * (p * tf.log(2.) - tf.linalg.logdet(S)) + \
        #log_multi_gamma(0.5 * nu, p) - \
            #0.5 * p * tf.log(kappa)
